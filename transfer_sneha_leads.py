...
def get_user_ids():
    """Get user IDs for Sneha and Hemlata"""
    try:
        ...
        return sneha_id[0], hemlata_id[0]
        
    except Exception as e:
        print(f"❌ Error getting user IDs: {e}")
        return None, None

def get_sneha_overdue_leads(sneha_id):
    """Get all of Sneha's leads with followup date before August 24th, 2025"""
    try:
        conn = psycopg2.connect(**DB_PARAMS)
        cur = conn.cursor()
        ...
        # Get all of Sneha's leads with followup date before August 24th
        cur.execute("""
            SELECT 
                id,
                customer_name,
        ...
        print(f"❌ Error getting Sneha's leads: {e}")
        return []

def calculate_new_followup_dates(leads_count, start_date):
    """Calculate new followup dates: 100 leads per day starting from start_date"""
    followup_dates = []
...
def transfer_leads_to_hemlata(sneha_id, hemlata_id, leads, start_date):
    """Transfer leads from Sneha to Hemlata with new followup dates"""
    try:
        conn = psycopg2.connect(**DB_PARAMS)
        cur = conn.cursor()
        ...
        print(f"🔄 Starting transfer of {len(leads)} leads from Sneha to Hemlata...")
        print(f"📅 Starting from: {start_date.strftime('%Y-%m-%d')}")
        print(f"📊 100 leads per day will be assigned")
        print("-" * 60)
            ...
            try:
                ...
                cur.execute("""
                    ...
                    SET 
                        ...
                        modified_at = %s
                    WHERE id = %s
                """, (hemlata_id, new_followup_date, datetime.now(pytz.timezone('Asia/Kolkata')), lead_id))
...
def main():
    print("🔄 Starting Sneha to Hemlata Lead Transfer Process")
    print("=" * 60)
    
    # Get user IDs
    sneha_id, hemlata_id = get_user_ids()
    if not sneha_id or not hemlata_id:
        print("❌ Failed to get user IDs")
        return
    
    print(f"✅ Found Sneha (ID: {sneha_id}) and Hemlata (ID: {hemlata_id})")
    
    # Get Sneha's overdue leads
    leads = get_sneha_overdue_leads(sneha_id)
    if not leads:
        print("❌ No leads found for transfer")
        return
    
    print(f"📊 Found {len(leads)} leads to transfer")
    ...
    print(f"\n⚠️  About to transfer {len(leads)} leads from Sneha to Hemlata")
    print(f"📅 Starting from: {start_date_ist.strftime('%Y-%m-%d')}")
    print(f"📊 100 leads per day will be assigned")
    
    # Transfer the leads
    transferred_count, errors = transfer_leads_to_hemlata(sneha_id, hemlata_id, leads, start_date_ist)
