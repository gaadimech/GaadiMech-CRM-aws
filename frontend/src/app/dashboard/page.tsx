  ...
  async function loadDashboard() {
    ...
    try {
      ...
      const metricsRes = await fetch(
        `${API_BASE}/api/dashboard/metrics?date=${selectedDate}`,
        { credentials: "include" }
      );
      if (metricsRes.ok) {
      ...
      const teamRes = await fetch(
        `${API_BASE}/api/dashboard/team-performance?date=${selectedDate}`,
        { credentials: "include" }
      );
      if (teamRes.ok) {
      ...
      const queueData = await fetchQueue({ date: selectedDate });
      setTodaysFollowups(queueData.items.slice(0, 10)); // Show first 10
    } catch (err) {
      ...
      <main className="mx-auto max-w-5xl px-4 py-6">
        ...
        {metrics && (
          <div className="grid grid-cols-2 gap-4 mb-6 lg:grid-cols-4">
            <div className="bg-purple-50 border border-purple-200 rounded-xl p-4">
              <p className="text-sm text-purple-700 mb-1">Today's Followups</p>
            ...
            <div className="bg-green-50 border border-green-200 rounded-xl p-4">
              <p className="text-sm text-green-700 mb-1">Initial Assignment</p>
            ...
            <div className="bg-pink-50 border border-pink-200 rounded-xl p-4">
              <p className="text-sm text-pink-700 mb-1">New Leads Today</p>
        ...
        <div className="bg-white rounded-xl border border-zinc-200 p-6 mb-6">
          ...
          {loading ? (
            ...
            <p className="text-sm text-zinc-500">No followups for today</p>
          ) : (
                ...
                >
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-zinc-500">
                      ...
                      {formatDate(item.followup_date)} {formatTime(item.followup_date)}
                    </p>
                    {item.remarks && (
                  ...
                  <ActionButtons lead={item} compact />
                </div>
              ))}
            </div>
          )}
