...
# Simplified database configuration for AWS RDS
DATABASE_URL = os.getenv("DATABASE_URL")

# AWS RDS fallback configuration - simplified
if not DATABASE_URL:
    ...
    # Simplified - use psycopg2 which is in requirements.txt
    DATABASE_URL = f"postgresql+psycopg2://{RDS_USER}:{RDS_PASSWORD}@{RDS_HOST}:{RDS_PORT}/{RDS_DB}"

# Ensure proper format - simplified
